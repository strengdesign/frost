...
<xsl:variable name="eventsNode" select="$rootNode/descendant::EventsFolder[@isDoc]" />
...
<div id="filters-results">    
    ...
    <form action="/news-events/events.aspx" method="post">
        <div class="greyBg_Holder">
            <div id="andor"><img src="/images/style/and-or.png" /></div>
            <h4>Filter by Industry:</h4> 
            
            <ul>
                <xsl:for-each select="$industriesNode/descendant::Industry[@isDoc and string(umbracoNaviHide) != '1']">
                    <li>
                    <label>
                        &nbsp;
...
<xsl:variable name="matchingArticles" select="$eventsNode/descendant::Event[@isDoc and ($qIndustry='' or frr:ListInList($qIndustry, eventIndustries)) and ($qService='' or frr:ListInList($qService, eventServices))]" />     	
        ...
        <xsl:if test="position() &gt;= (($page - 1)*$pageSize) + 1 and position() &lt;= $page*$pageSize">
          <li class="">
            <xsl:variable name="tagsPreNodes">
                <xsl:variable name="industryTags" select="eventIndustries" />
            ...
            <xsl:variable name="tagsServicePreNodes">
                <xsl:variable name="serviceTags" select="eventServices" />
            ...
            <strong><xsl:value-of select="umbraco.library:FormatDateTime(articlePublishDate, 'MMMM d, yyyy')"/></strong>
            
            <p>
				<xsl:choose>
                	<xsl:when test="string-length(eventTeaser)>0">
						<p>
							<xsl:value-of select="eventTeaser" /> 
							<br /><a href="{umbraco.library:NiceUrl(@id)}">Read More ></a>
						</p> 
					</xsl:when>          
					<xsl:otherwise>			
						<p>
							<xsl:value-of select="umbraco.library:TruncateString(eventDescription, 150, '...')" disable-output-escaping="yes"/>
							<br /><a href="{umbraco.library:NiceUrl(@id)}">Read More ></a>
						</p>
				    </xsl:otherwise>
				</xsl:choose> 
