...
<xsl:variable name="newslettersNode" select="$rootNode/descendant::NewslettersFolder[@isDoc]" />
...
<div id="filters-results">
    ...
    <form action="/news-events/e-newsletters.aspx" method="post">
...
<xsl:variable name="matchingArticles" select="$newslettersNode/descendant::Newsletter[@isDoc and ($qIndustry='' or frr:ListInList($qIndustry, newsletterIndustries)) and ($qService='' or frr:ListInList($qService, newsletterServices)) ]" />     	
            ...
            <xsl:if test="position() &gt;= (($page - 1)*$pageSize) + 1 and position() &lt;= $page*$pageSize">
              <li class="">
                <xsl:variable name="tagsPreNodes">
                    <xsl:variable name="industryTags" select="newsletterIndustries" />
                ...
                <xsl:variable name="tagsServicePreNodes">
                    <xsl:variable name="serviceTags" select="newsletterServices" />
                ...
                <p><xsl:value-of select="newsletterTeaser" /><br /><a href="{umbraco.library:NiceUrl(@id)}">Read More ></a></p>
