...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="searchUpper" select="PS.XSLTsearch:uppercase(string($search))"/>
   ...
   <!-- Date Parameters - JG 11/28/2012 -->
  <xsl:variable name="searchFrom">
    <xsl:choose>
      <!-- form field value, if present -->
      <xsl:when test="string(umbraco.library:RequestForm('searchFromDate')) != ''">
        <xsl:value-of select="PS.XSLTsearch:cleanSearchTerm(PS.XSLTsearch:escapeString(string(umbraco.library:RequestForm('searchFromDate'))))" />
      </xsl:when>
      <!-- querystring value, if present -->
      <xsl:when test="string(umbraco.library:RequestQueryString('searchFromDate')) != ''">
        <xsl:value-of select="PS.XSLTsearch:cleanSearchTerm(PS.XSLTsearch:escapeString(string(umbraco.library:RequestQueryString('searchFromDate'))))" />
      </xsl:when>
      <!-- no value -->
      <xsl:otherwise>
        <xsl:value-of select="'1/1/1901'"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
  
  <xsl:variable name="searchTo">
    <xsl:choose>
      <!-- form field value, if present -->
      <xsl:when test="string(umbraco.library:RequestForm('searchToDate')) != ''">
        <xsl:value-of select="PS.XSLTsearch:cleanSearchTerm(PS.XSLTsearch:escapeString(string(umbraco.library:RequestForm('searchToDate'))))" />
      </xsl:when>
      <!-- querystring value, if present -->
      <xsl:when test="string(umbraco.library:RequestQueryString('searchToDate')) != ''">
        <xsl:value-of select="PS.XSLTsearch:cleanSearchTerm(PS.XSLTsearch:escapeString(string(umbraco.library:RequestQueryString('searchToDate'))))" />
      </xsl:when>
      <!-- no value -->
      <xsl:otherwise>
        <xsl:value-of select="'1/1/2099'"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
  <!-- / Date Parameters - JG 11/28/2012 -->
  ...
  <xsl:template name="search">
    ...
    <xsl:variable name="possibleNodes" select="$items/descendant-or-self::*[
       ...
       ][name()='NewsArticle' or name()='Event' or name()='Newsletter']
       [normalize-space(articlePublishDate)][umbraco.library:DateGreaterThan(articlePublishDate, $searchFrom) and umbraco.library:DateGreaterThan($searchTo, articlePublishDate)]"/>  
    ...
    <xsl:variable name="matchedNodesIdList">
      <xsl:call-template name="booleanAndMatchedNodes">
        <xsl:with-param name="yetPossibleNodes" select="msxml:node-set($possibleNodes)"/>
          ...
          <xsl:for-each select="$matchedNodes">
            ...
            <xsl:if test="position() &gt;= $startMatch and position() &lt;= $endMatch">
              ...
              <div class="xsltsearch_result">
                <p class="xsltsearch_result_title" style="padding-bottom: 0px;">
                  ...
                  <!-- page name and url -->
                  <a href="{umbraco.library:NiceUrl(@id)}" id="search-link">
                    <xsl:value-of select="@nodeName"/>
                  </a>
                  <span style="color: #999000"><xsl:value-of select="umbraco.library:NiceUrl(@id)" /></span>
