...
<div class="paddingDiv">
	...
	<h1 class="styled"><xsl:value-of select="$currentPage/eventTitle" /></h1>
	
    <strong class="large"><xsl:value-of select="umbraco.library:FormatDateTime($currentPage/articlePublishDate, 'MMMM d, yyyy')"/></strong>
    
    <div id="industry-tags">
        <xsl:variable name="preNodes">
            <xsl:variable name="industries" select="$currentPage/eventIndustries" />
    ...
    <div id="services-tags">
        <xsl:variable name="preNodes">
            <xsl:variable name="services" select="$currentPage/eventServices" />
    ...
    <p><xsl:value-of select="$currentPage/eventDescription" disable-output-escaping="yes" /></p>
